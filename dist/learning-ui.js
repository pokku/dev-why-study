...
export function createHomePortal(){
 const section=node('section','learning-portal');section.innerHTML=`<div class="learning-hero"><div><p class="micro-label">ひとつの「なぜ？」から、世界がつながる。</p><h1>触ってみたら、<br>数式に意味が見えてきた。</h1><p>音、光、電波、宇宙、そして暗号やAIまで。${labs.length}の体験から、身近な仕組みと学校の勉強を行き来しよう。</p><div class="learning-nav"><a href="#journey=sound">音の正体を追いかける →</a><a href="#why">単元ごとの「なぜ習う？」</a><a href="#labs">${labs.length}の体験から選ぶ</a><a href="#notebook">発見ノート ${count()}</a></div></div><div class="learning-hero-map" aria-hidden="true"><span>ギターの音</span><i>↓ なぜ音色が違う？</i><span>波を足す・分ける</span><i>↓ どう表せる？</i><span>sin · Σ · フーリエ</span><i>↓ 同じ考え方で</i><span>写真や電波へ</span></div></div><div class="learning-section-title"><h2>疑問でつながる、${journeys.length}つの探索ルート</h2><p>順番に進んでも、気になるところからでも。</p></div><div class="journey-grid"></div>`;
